...
func get_commands() -> Dictionary:
	return {
		...
		"create_tween": _create_tween,
		"tween_property": _tween_property,
		"tween_callback": _tween_callback,
		"tween_interval": _tween_interval,
		"tween_method": _tween_method,
		"get_active_tweens": _get_active_tweens,
	}
...
func _record_frames(params: Dictionary) -> Dictionary:
	var count: int = clampi(optional_int(params, "count", 60), 1, 600)
	var frame_interval: int = maxi(optional_int(params, "frame_interval", 2), 1)
	var half_resolution: bool = optional_bool(params, "half_resolution", true)
	var output_dir: String = optional_string(params, "output_dir", "user://mcp_recorded_frames")

	# Dynamic timeout: (frames * interval) / 60fps + overhead, max 120s
...
## ── Tween tools (runtime IPC) ────────────────────────────────────────────────

func _create_tween(params: Dictionary) -> Dictionary:
	var res := require_string(params, "node_path"); if res[1]: return res[1]
	return await _send_game_command("tween_create", {
		"node_path": res[0],
		"loops": optional_int(params, "loops", 0),
		"parallel": optional_bool(params, "parallel", false),
		"pause_mode": optional_string(params, "pause_mode", "inherit"),
	}, 5.0)


func _tween_property(params: Dictionary) -> Dictionary:
	var res := require_string(params, "tween_id"); if res[1]: return res[1]
	var np  := require_string(params, "node_path"); if np[1]: return np[1]
	var prop := require_string(params, "property"); if prop[1]: return prop[1]
	if not params.has("target_value"): return error_invalid_params("target_value is required")
	return await _send_game_command("tween_property", {
		"tween_id": res[0], "node_path": np[0], "property": prop[0],
		"target_value": params["target_value"],
		"duration": float(params.get("duration", 1.0)),
		"ease": optional_string(params, "ease", "in_out"),
		"trans": optional_string(params, "trans", "linear"),
	}, 5.0)


func _tween_callback(params: Dictionary) -> Dictionary:
	var res := require_string(params, "tween_id"); if res[1]: return res[1]
	var code := require_string(params, "code"); if code[1]: return code[1]
	return await _send_game_command("tween_callback", {
		"tween_id": res[0], "code": code[0],
		"delay": float(params.get("delay", 0.0)),
	}, 5.0)


func _tween_interval(params: Dictionary) -> Dictionary:
	var res := require_string(params, "tween_id"); if res[1]: return res[1]
	return await _send_game_command("tween_interval", {
		"tween_id": res[0],
		"time": float(params.get("time", 1.0)),
	}, 5.0)


func _tween_method(params: Dictionary) -> Dictionary:
	var res  := require_string(params, "tween_id"); if res[1]: return res[1]
	var np   := require_string(params, "node_path"); if np[1]: return np[1]
	var meth := require_string(params, "method"); if meth[1]: return meth[1]
	if not params.has("from_value"): return error_invalid_params("from_value is required")
	if not params.has("to_value"):   return error_invalid_params("to_value is required")
	return await _send_game_command("tween_method", {
		"tween_id": res[0], "node_path": np[0], "method": meth[0],
		"from_value": params["from_value"], "to_value": params["to_value"],
		"duration": float(params.get("duration", 1.0)),
	}, 5.0)


func _get_active_tweens(params: Dictionary) -> Dictionary:
	var res := require_string(params, "node_path"); if res[1]: return res[1]
	return await _send_game_command("tween_list", {"node_path": res[0]}, 5.0)
