...
func get_commands() -> Dictionary:
	return {
		...
		"get_selected_files": _get_selected_files,
	}
...
func _create_directory(params: Dictionary) -> Dictionary:
	var result := require_string(params, "path")
	if result[1] != null:
		return result[1]
	var path: String = result[0]
	...
	if DirAccess.dir_exists_absolute(path):
		return success({"path": path, "message": "Directory already exists", "created": false})

	var err := DirAccess.make_dir_recursive_absolute(path)
	if err != OK:
		return error_internal("Failed to create directory: %s" % error_string(err))
...
func _delete_file(params: Dictionary) -> Dictionary:
	...
	if not FileAccess.file_exists(path) and not DirAccess.dir_exists_absolute(path):
		return error_not_found("File or directory '%s'" % path)

	var err: int
	if DirAccess.dir_exists_absolute(path):
		# Godot 4 doesn't have a recursive delete in DirAccess for safety,
		# but EditorInterface has get_resource_filesystem().
		# Actually, we can use OS.move_to_trash() or DirAccess.remove_absolute() for empty dirs.
		# To be safe and compatible with Godot's way, we should probably use DirAccess.
		err = _delete_recursive(path)
	else:
		err = DirAccess.remove_absolute(path)

	if err != OK:
		return error_internal("Failed to delete '%s': %s" % [path, error_string(err)])
...
func _rename_file(params: Dictionary) -> Dictionary:
	var result := require_string(params, "from")
	if result[1] != null:
		return result[1]
	var from_path: String = result[0]

	var result2 := require_string(params, "to")
	if result2[1] != null:
		return result2[1]
	var to_path: String = result2[0]

	if not FileAccess.file_exists(from_path) and not DirAccess.dir_exists_absolute(from_path):
		return error_not_found("Source '%s'" % from_path)

	var err := DirAccess.rename_absolute(from_path, to_path)
	if err != OK:
		return error_internal("Failed to rename/move: %s" % error_string(err))

	get_editor().get_resource_filesystem().scan()
	return success({"from": from_path, "to": to_path, "renamed": true})


func _get_selected_files(_params: Dictionary) -> Dictionary:
	var selected := get_editor().get_selected_paths()
	return success({
