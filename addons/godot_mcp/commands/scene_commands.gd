...
func get_commands() -> Dictionary:
	return {
		...
		"save_branch_as_scene": _save_branch_as_scene,
	}
...
func _save_branch_as_scene(params: Dictionary) -> Dictionary:
	...
	if result2[1] != null:
		return result2[1]
	var path: String = result2[0]

	var node := find_node_by_path(node_path)
	if node == null:
		return error_not_found("Node '%s'" % node_path)

	if node == get_edited_root():
		return error_invalid_params("Cannot save the root node as a branch (use save_scene instead)")

	# Pack the branch
	var scene := PackedScene.new()
	# We need to temporarily set the node's owner to itself for packing to work correctly as a standalone scene
	var old_owner := node.owner
	
	# Recursively set owner to node for the branch
	_set_owner_recursive_branch(node, node)
	
	var err := scene.pack(node)
	
	# Restore owner
	_set_owner_recursive_branch(node, old_owner)
	node.owner = old_owner

	if err != OK:
		return error_internal("Failed to pack branch: %s" % error_string(err))

	err = ResourceSaver.save(scene, path)
	if err != OK:
		return error_internal("Failed to save scene: %s" % error_string(err))

	# Optional: Replace node with instance of new scene
	if optional_bool(params, "replace_with_instance", true):
		...
		var undo_redo := get_undo_redo()
		undo_redo.create_action("MCP: Save Branch as Scene")
		undo_redo.add_do_method(parent, "remove_child", node)
		undo_redo.add_do_method(parent, "add_child", instance)
		undo_redo.add_do_method(parent, "move_child", instance, index)
		undo_redo.add_do_method(instance, "set_owner", get_edited_root())
		undo_redo.add_do_reference(instance)
		
		undo_redo.add_undo_method(parent, "remove_child", instance)
		undo_redo.add_undo_method(parent, "add_child", node)
		undo_redo.add_undo_method(parent, "move_child", node, index)
		undo_redo.add_undo_method(node, "set_owner", get_edited_root())
		undo_redo.add_undo_reference(node)
		undo_redo.commit_action()

	get_editor().get_resource_filesystem().scan()
	return success({"path": path, "node_path": node_path, "saved": true})


func _set_owner_recursive_branch(node: Node, new_owner: Node) -> void:
	for child in node.get_children():
		child.owner = new_owner
		_set_owner_recursive_branch(child, new_owner)
