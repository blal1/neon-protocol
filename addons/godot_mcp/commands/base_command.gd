...
## Get required array param
func require_array(params: Dictionary, key: String) -> Array:
	if not params.has(key) or not params[key] is Array:
		return [null, error_invalid_params("Missing required parameter: %s (array)" % key)]
	return [params[key] as Array, null]


## Get optional array param with default
func optional_array(params: Dictionary, key: String, default: Array = []) -> Array:
	if params.has(key) and params[key] is Array:
		return params[key] as Array
	return default
