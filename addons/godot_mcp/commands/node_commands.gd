...
func get_commands() -> Dictionary:
	return {
		...
		"find_nodes_by_property": _find_nodes_by_property,
		"get_node_resource": _get_node_resource,
	}
...
func _find_nodes_by_property(params: Dictionary) -> Dictionary:
	var result := require_string(params, "property")
	if result[1] != null:
		return result[1]
	var property: String = result[0]
	...
	var matches: Array = []
	_find_by_property_recursive(root, root, property, value, matches)
...
func _find_by_property_recursive(node: Node, root: Node, property: String, value: Variant, matches: Array) -> void:
	if property in node:
		var node_val = node.get(property)
		var match_found := false

		if typeof(node_val) == typeof(value):
			match_found = (node_val == value)
		else:
			# Try string comparison as fallback for complex types
			match_found = (str(node_val) == str(value))
	...
	for child in node.get_children():
		_find_by_property_recursive(child, root, property, value, matches)


func _get_node_resource(params: Dictionary) -> Dictionary:
	var result := require_string(params, "node_path")
	if result[1] != null:
		return result[1]
	var node_path: String = result[0]
	...
	if not property in node:
		return error_not_found("Property '%s' on node" % property)

	var res: Variant = node.get(property)
	if res == null:
		return success({"node_path": node_path, "property": property, "resource": null})

	if not res is Resource:
		return error_invalid_params("Property '%s' is not a Resource (is %s)" % [property, type_string(typeof(res))])

	var props: Dictionary = {}
	for prop_info in res.get_property_list():
		var p_name: String = prop_info["name"]
		if prop_info["usage"] & PROPERTY_USAGE_EDITOR:
			if p_name.begins_with("_") or p_name in ["script", "resource_local_to_scene", "resource_name", "resource_path"]:
				continue
			props[p_name] = PropertyParser.serialize_value(res.get(p_name))
