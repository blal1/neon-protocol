...
func get_commands() -> Dictionary:
	return {
		...
		"get_selected_nodes": _get_selected_nodes,
		"select_nodes": _select_nodes,
		"get_class_info": _get_class_info,
		"get_editor_settings": _get_editor_settings,
	}
...
func _get_editor_screenshot(params: Dictionary) -> Dictionary:
	...
	# P2.5: Downscale to fit max_width × max_height (default 1280×720) to cap context size
	...
	if max_w > 0 and max_h > 0:
		...
		if iw > max_w or ih > max_h:
			var scale := minf(float(max_w) / iw, float(max_h) / ih)
			image.resize(int(iw * scale), int(ih * scale), Image.INTERPOLATE_BILINEAR)
...
func _get_game_screenshot(params: Dictionary) -> Dictionary:
	...
	# P2.5: Downscale to fit max_width × max_height (default 1280×720)
	var max_w: int = params.get("max_width", 1280)
	var max_h: int = params.get("max_height", 720)
	if max_w > 0 and max_h > 0:
		var iw := image.get_width()
...
"""
	...
	for func_line in class_funcs:
		wrapped_code += func_line + "\n"
	if class_funcs.size() > 0:
		wrapped_code += "\n"

	wrapped_code += "func run() -> Variant:\n"
...
func _get_selected_nodes(_params: Dictionary) -> Dictionary:
	var selection := get_editor().get_selection()
	var selected_nodes := selection.get_selected_nodes()
	var root := get_edited_root()

	var paths: Array = []
	for node in selected_nodes:
		if root and (node == root or root.is_ancestor_of(node)):
			paths.append(str(root.get_path_to(node)))
		else:
			paths.append(str(node.get_path()))

	return success({
		"selected_node_paths": paths,
		"count": paths.size()
	})


func _select_nodes(params: Dictionary) -> Dictionary:
	var result := require_array(params, "node_paths")
	if result[1] != null:
		return result[1]
	var node_paths: Array = result[0]

	var clear_selection: bool = optional_bool(params, "clear_selection", true)
	var selection := get_editor().get_selection()

	if clear_selection:
		selection.clear()

	var success_paths: Array = []
	var failed_paths: Array = []

	for path in node_paths:
		var node := find_node_by_path(path)
		if node:
	...
	return success({
		"selected": success_paths,
		"failed": failed_paths,
		"cleared_previous": clear_selection
	})


func _get_class_info(params: Dictionary) -> Dictionary:
	var result := require_string(params, "class_name")
	if result[1] != null:
		return result[1]
	var cls_name: String = result[0]

	if not ClassDB.class_exists(cls_name):
		return error_not_found("Class '%s'" % cls_name)

	var info := {
		"class_name": cls_name,
		"inherits": ClassDB.get_parent_class(cls_name),
		"is_instantiatable": ClassDB.can_instantiate(cls_name),
	}

	# Properties
	var properties: Array = []
	for prop in ClassDB.class_get_property_list(cls_name, true):
	...
	for method in ClassDB.class_get_method_list(cls_name, true):
		var m_info := {
			"name": method["name"],
			"return": type_string(method["return"]["type"]),
			"args": []
		}
	...
	for sig in ClassDB.class_get_signal_list(cls_name, true):
		...
		signals.append(s_info)
	info["signals"] = signals

	# Constants
	var constants: Dictionary = {}
	for const_name in ClassDB.class_get_integer_constant_list(cls_name, true):
		constants[const_name] = ClassDB.class_get_integer_constant(cls_name, const_name)
	info["constants"] = constants

	# Enums
	var enums: Dictionary = {}
	for enum_name in ClassDB.class_get_enum_list(cls_name, true):
		enums[enum_name] = Array(ClassDB.class_get_enum_constants(cls_name, enum_name, true))
	info["enums"] = enums

	return success(info)


func _get_editor_settings(params: Dictionary) -> Dictionary:
	var setting_key: String = optional_string(params, "setting", "")
	var settings := get_editor().get_editor_settings()

	if not setting_key.is_empty():
		if settings.has_setting(setting_key):
			return success({
				"setting": setting_key,
				"value": str(settings.get_setting(setting_key))
			})
		else:
			return error_not_found("Editor setting '%s'" % setting_key)
	...
	for prop in settings.get_property_list():
		...
		if filter.is_empty() or name.contains(filter):
			result[name] = str(settings.get_setting(name))
