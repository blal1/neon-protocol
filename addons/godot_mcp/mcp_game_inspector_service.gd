...
enum State { IDLE, CAPTURING_FRAMES, RECORDING_FRAMES, MONITORING, RECORDING, MOVING_TO, WATCHING_SIGNALS }

var _state := State.IDLE
var _pending_command: bool = false  # Crash recovery flag
var _busy: bool = false             # IPC busy guard (P1.4)

# Tween registry (tween_id -> Tween)
var _tweens: Dictionary = {}
var _tween_next_id: int = 1
...
# record_frames state (disk-saving variant, up to 600 frames)
var _record_count: int = 0
var _record_frame_interval: int = 2
var _record_frame_counter: int = 0
var _record_half_res: bool = true
var _record_output_dir: String = "user://mcp_recorded_frames"
var _record_saved_paths: Array = []  # Array of absolute file paths
var _record_node_path: String = ""
var _record_node_props: Array = []
var _record_frame_data: Array = []  # Per-frame node snapshots
...
func _handle_request() -> void:
	...
	# P1.4: Guard against concurrent IPC calls.
	# Async commands (capture_frames, record_frames, monitor, etc.) clear _busy themselves
	# at completion. Sync commands clear it immediately after returning.
	if _busy:
		_write_response({
			"error": "busy",
			"message": "A command is already in progress. Please wait and retry."
		})
		return
	_busy = true
	...
	match command:
		...
		"tween_create":
			_cmd_tween_create(params)
		"tween_property":
			_cmd_tween_property(params)
		"tween_callback":
			_cmd_tween_callback(params)
		"tween_interval":
			_cmd_tween_interval(params)
		"tween_method":
			_cmd_tween_method(params)
		"tween_list":
			_cmd_tween_list(params)
		_:
...
# ── record_frames (disk-saving, up to 600 frames) ─────────────────────────────

func _cmd_record_frames(params: Dictionary) -> void:
	_pending_command = false  # Async command — don't trigger crash recovery
	_record_count = clampi(params.get("count", 60), 1, 600)
	_record_frame_interval = maxi(params.get("frame_interval", 2), 1)
	_record_half_res = params.get("half_resolution", true)
	_record_output_dir = params.get("output_dir", "user://mcp_recorded_frames")
	...
	var abs_dir := ProjectSettings.globalize_path(_record_output_dir)
	if not DirAccess.dir_exists_absolute(abs_dir):
		DirAccess.make_dir_recursive_absolute(abs_dir)

	_record_saved_paths.clear()
	# P1.3: prime counter to threshold so _process captures frame 0 on first tick.
	# Do NOT call _record_one_frame() here — that would double-capture when frame_interval=1.
	_record_frame_counter = _record_frame_interval
...
func _record_one_frame() -> void:
	...
	# Build file path: <output_dir>/frame_NNNN.png
	var frame_index: int = _record_saved_paths.size()
	var filename := "frame_%04d.png" % frame_index
	var res_path := _record_output_dir.path_join(filename)
	var abs_path := ProjectSettings.globalize_path(res_path)

	var err := image.save_png(abs_path)
	if err == OK:
		_record_saved_paths.append(abs_path)
	else:
		push_warning("[MCP] record_frames: failed to save %s (error %d)" % [abs_path, err])
	...
	if not _record_node_path.is_empty() and not _record_node_props.is_empty():
		...
		if node:
			for prop_name in _record_node_props:
				snap[prop_name] = _serialize_value(node.get(prop_name))
...
func _finish_record_frames() -> void:
	...
	var response := {
		"file_paths": _record_saved_paths,
		"count": _record_saved_paths.size(),
		"output_dir": ProjectSettings.globalize_path(_record_output_dir),
		"width": w,
		"height": h,
...
func _write_response(data: Dictionary) -> void:
	_pending_command = false
	_busy = false  # P1.4: always release busy guard on any response write
...
func _parse_ease(s: String) -> Tween.EaseType:
	match s.to_lower():
...
func _parse_trans(s: String) -> Tween.TransitionType:
	match s.to_lower():
...
func _cmd_tween_create(params: Dictionary) -> void:
	var node_path: String = params.get("node_path", "")
	var node: Node = get_tree().current_scene.get_node_or_null(NodePath(node_path))
	if node == null:
		_write_response({"error": "Node not found: %s" % node_path})
		return
	var tw: Tween = node.create_tween()
	tw.set_loops(int(params.get("loops", 0)))
	if bool(params.get("parallel", false)): tw.set_parallel(true)
	var pause_mode: String = str(params.get("pause_mode", "inherit"))
	match pause_mode.to_lower():
		"stop":    tw.set_pause_mode(Tween.TWEEN_PAUSE_STOP)
		"process": tw.set_pause_mode(Tween.TWEEN_PAUSE_PROCESS)
		_:         tw.set_pause_mode(Tween.TWEEN_PAUSE_BOUND)
	var id: String = str(_tween_next_id)
	_tween_next_id += 1
	_tweens[id] = tw
	_write_response({"tween_id": id, "node_path": node_path, "loops": int(params.get("loops", 0))})

func _cmd_tween_property(params: Dictionary) -> void:
	var tid: String = str(params.get("tween_id", ""))
	if not _tweens.has(tid):
		_write_response({"error": "tween_id '%s' not found. Call create_tween first." % tid})
		return
	var tw: Tween = _tweens[tid]
	var node_path: String = params.get("node_path", "")
	var node: Node = get_tree().current_scene.get_node_or_null(NodePath(node_path))
	if node == null:
		_write_response({"error": "Node not found: %s" % node_path})
	...
	var target = params.get("target_value")
	var prop_tweener: PropertyTweener = tw.tween_property(node, prop, target, duration)
	prop_tweener.set_ease(_parse_ease(str(params.get("ease", "in_out"))))
	prop_tweener.set_trans(_parse_trans(str(params.get("trans", "linear"))))
	_write_response({"tween_id": tid, "property": prop, "duration": duration})

func _cmd_tween_callback(params: Dictionary) -> void:
	var tid: String = str(params.get("tween_id", ""))
	if not _tweens.has(tid):
		_write_response({"error": "tween_id '%s' not found." % tid})
		return
	var tw: Tween = _tweens[tid]
	...
	if err != OK:
		_write_response({"error": "Expression parse error: %s" % expr.get_error_text()})
		return
	var cb: Callable = func(): expr.execute([], self)
	tw.tween_callback(cb)
	_write_response({"tween_id": tid, "code": code})

func _cmd_tween_interval(params: Dictionary) -> void:
	var tid: String = str(params.get("tween_id", ""))
	if not _tweens.has(tid):
		_write_response({"error": "tween_id '%s' not found." % tid})
		return
	var tw: Tween = _tweens[tid]
	var time: float = float(params.get("time", 1.0))
	tw.tween_interval(time)
	_write_response({"tween_id": tid, "interval": time})

func _cmd_tween_method(params: Dictionary) -> void:
	var tid: String = str(params.get("tween_id", ""))
	if not _tweens.has(tid):
		_write_response({"error": "tween_id '%s' not found." % tid})
		return
	...
	var from_val = params.get("from_value")
	var to_val   = params.get("to_value")
	var duration: float = float(params.get("duration", 1.0))
	tw.tween_method(Callable(node, method), from_val, to_val, duration)
	_write_response({"tween_id": tid, "method": method, "duration": duration})

func _cmd_tween_list(params: Dictionary) -> void:
	var active: Array = []
	for tid in _tweens:
		var tw: Tween = _tweens[tid]
		active.append({"tween_id": tid, "is_running": tw.is_running(), "is_valid": tw.is_valid()})
	# Clean up dead tweens
	for tid in _tweens.keys():
		if not (_tweens[tid] as Tween).is_valid():
			_tweens.erase(tid)
	_write_response({"tweens": active, "count": active.size()})
